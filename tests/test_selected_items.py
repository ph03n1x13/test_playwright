...
def test_add_products_to_cart(browser_context, request):
    ...
    web_page.page.locator(productpage.ADD_BUTTONS).nth(0).click()
    selected_items.append(web_page.page.locator(productpage.PRODUCTS).nth(0).text_content())

    web_page.page.locator(productpage.ADD_BUTTONS).nth(1).click()
    selected_items.append(web_page.page.locator(productpage.PRODUCTS).nth(2).text_content())

    web_page.page.locator(productpage.ADD_BUTTONS).nth(2).click()
    selected_items.append(web_page.page.locator(productpage.PRODUCTS).nth(4).text_content())

    logging.info(selected_items)

    nodeid = request.node.nodeid.replace("::", "_").replace("/", "_")
    file_name = file_path.screenshot_path(f'{nodeid}.png')
    web_page.page.screenshot(path=file_name, full_page=True)
...
def test_go_to_your_cart_page(browser_context, request):
    ...
    web_page.page.locator(productpage.SHOPPING_CART_LOGO).click()
    expect(web_page.page.get_by_text('Your Cart')).to_be_visible()
...
def test_added_products_ok(browser_context, request):
    web_page = browser_context["web_page"]
    selected_items = browser_context["selected_items"]
    cart_items = web_page.page.locator(cartpage.CART_ITEM_NAMES).all_text_contents()
